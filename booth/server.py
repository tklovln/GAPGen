...
BOOTH_LEVEL_HINT = (
    ...
    "\n⚠️ 目標數量鐵則：紙箱(Crt)/木桶(Barrel)/水漥(Puddle)/三角錐這類障礙，"
    "「打爆一個算一個」，所以『目標數量』絕對不能超過你「實際放在盤面上的該物件數量」。"
    "請先想好盤面要放幾個，再把目標設成『等於或略少於』那個數量；"
    "唯一例外是有 spawner 會持續生成的物件（如障礙雨的木桶）才可以設比盤面初始多。"
    "千萬不要『目標 12 個紙箱、盤面卻只放 8 個』這種一定過不了的關。）"
)
BOOTH_LEVEL_HINT_SHAPE = (
    ...
    "\n⚠️ 目標數量鐵則：紙箱/木桶/水漥這類障礙『打爆一個算一個』，目標數量絕不能超過盤面實際放的該物件數量"
    "（除非有 spawner 持續生成）。先想好盤面放幾個再設目標，不要做成『目標比盤面多』一定過不了的關。）"
)
