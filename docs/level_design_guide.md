...
> ⚠️ **鐵則：障礙物目標數 = 盤面上該物件的「個數」，血量(HP)一律不影響計數。**
> 引擎是「清掉 1 個物件 = 目標 +1」，不是「每滴血 +1」。

**所有障礙物**（Crt/Barrel/TrafficCone/SalmonCan/**Puddle** 等）：
- 目標數 = 盤面上該物件的「個數」（**health / lv 不影響計數**）
- 例：放了 20 個 Crt2 → 目標設 20（或更少）
- 例：放了 **8 個 Puddle_lv2 → 目標設 8**（不是 16；血量 2 滴只是要打 2 次才清掉，但仍只算 1 個）

**Puddle（水漥）特別注意**：
- ❌ 不要寫成 格子數 × 血量（這是常見錯誤，會把 8 個設成 16）
- ✅ 就是格子數本身：8 格 = 目標 8

**「所有出現的障礙物都要列入目標」**：
- 大部分關卡，**盤面上有出現的每一種障礙物都應該列進 `goals`**（讓玩家把它們全部清掉才算過關）。
- 不要只挑一種當目標、其他障礙物放著不算 —— 除非該障礙物是純裝飾/輔助（少見）。
...
### 7.7 異形盤面（含 void / G、g、愛心、十字 等形狀）的障礙物可消除性 ⚠️

用 void 挖成特殊形狀時最容易做出「障礙物根本消不掉」的死關，務必逐一檢查：

- **每個吃三消的障礙物（Crt/Barrel/TrafficCone/Puddle 等）旁邊都要有可產生相鄰消除的空地**：它的上下左右要有足夠可移動元素格，玩家才能在旁邊湊出三消把它清掉。不要把障礙物塞進被 void/其他障礙包住、旁邊湊不出消除的角落或細縫。
- **唯一例外 —— 刻意設計成「用道具消」**：若某障礙物（尤其 SalmonCan 只吃道具）是故意要玩家用道具（TNT 5×5 / 火箭整排 / 紙飛機）清，那就必須把它放在「道具效果打得到」的位置，並確保盤面留得下合成那個道具的空間。
- **形狀筆畫盡量 ≥ 2 格寬**：只有 1 格寬的細筆畫，裡面的障礙物常常旁邊湊不出三消 → 變成死關。
...
- ⚠️ **異形盤面（有 void）務必檢查 spawner 出口與落道**：spawn_cols 指定欄的 row 0 以及它「整條往下的路徑」不能被 void 或固定障礙物堵住；否則障礙物生不出來、或卡在半空 → 下方一直留著填不滿的空位。異形盤面挑 spawn_cols 時，選「從最上到最下都暢通」的欄。
- 只有 Barrel 和 TrafficCone 適合作為 spawner 生成物（它們是可移動的）
- 目標數量可以大於盤面初始數量（spawner 會持續補充）
- 引擎有飽和機制：當盤面數量 + 已消除數 >= 目標數時，spawner 自動停止
- **生成機率務必設 33%~50%**：`set_ratio=1, total_weight=3`（33%）或 `set_ratio=1, total_weight=2`（50%）。讓障礙物「邊掉邊消」、有動態平衡的樂趣。
- ⚠️ **嚴禁 100%（set_ratio = total_weight，例如 3/3）**：那會「每個空格都生障礙」→ 盤面瞬間被灌爆、幾乎無法操作（連 AI 都過不了、勝率 0%）。障礙雨的樂趣是「邊消邊掉」，不是「淹沒」。
- **目標數要可達成**：spawner 目標數建議 ≤ max_steps × 0.6，並留足步數讓玩家清完（否則必定打不完）。
...
{
  "name": "木桶雨",
  "rows": 11, "cols": 8, "num_colors": 4, "max_steps": 32,
  "goals": {"Barrel": 18, "Puddle_lv2": 48},
  "board": {
    "middle": [
      [null,null,null,null,null,null,null,null],
      [null,null,null,null,null,null,null,null],
